// import { defineTextStyles } from '@chakra-ui/react'

// export const textStyles = defineTextStyles({

//   // Landing Page
//   headingLP:{
//     description: 'For Heading in Landing Page',
//     value: {
//       fontFamily: 'var(--font-playfair-display)',
//       fontWeight: '600',
//       fontSize: { base: "50px", md: "50px" },
//       lineHeight: "24px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   subHeadingLP:{
//     description: "For Sub Heading in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "400",
//       fontSize: "35px",
//       lineHeight: "40px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   subHeadingLP2:{
//     description: "For Sub Heading in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "35px",
//       lineHeight: "40px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   textLP:{
//     description: "For Sub Heading in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: "18px",
//       lineHeight: "40px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   textLP2:{
//     description: "For Sub Heading in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "18px",
//       lineHeight: "40px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   headLPGrid:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "20px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "uppercase",
//     },
//   },
//   textLPGrid:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: "16px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   headingLPLogin:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "20px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   textLPLogin:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: "14px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   googleLPLogin:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: "14px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   miniHeadingLPFeat:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "14px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "Uppercase",
//     },
//   },
//   headingLPFeat:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "16px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   textLPFeat:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: "14px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   headingLPAbout:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "20px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "uppercase",
//     },
//   },
//   subHeadingLPAbout:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "18px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "uppercase",
//     },
//   },
//   textLPAbout:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: "16px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   textLPAbout2:{
//     description: "For Heading Feature Grid in Landing Page",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "800",
//       fontSize: "16px",
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "uppercase",
//     },
//   },

//   // Navbar
//   headingNav:{
//     description: "For Heading in Navbar",
//     value: {
//       fontFamily: "var(--font-playfair-display)",
//       fontWeight: "600",
//       fontSize: { base: "22px", sm: "26px", md: "28px", lg: "30px", xl: "40px" },
//       lineHeight: "24px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   subheadingNav:{
//     description: "For Sub Heading in Navbar",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { md: "12px", lg: "14px", xl: "16px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   selectNav:{
//     description: "For Text Menu in Navbar",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "14px", sm: "14px", md: "14px", lg: "14px", xl: "14px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },

//   // Time Widget
//   timeWidgetNum:{
//     description: "For Number in Time Widget",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { base: "60px", sm: "60px", md: "60px", lg: "60px", xl: "60px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   timeWidgetText:{
//     description: "For Text in Time Widget",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { base: "24px", sm: "28px", md: "28px", lg: "28px", xl: "35px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },

//   // Status Widget
//   headingStatusWidget:{
//     description: "For Text in Time Widget",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: { base: "600", sm: "600", md: "700", lg: "700", xl: "700" },
//       fontSize: { md: "16px", lg: "16px", xl: "16px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   contentStatusWidget:{
//     description: "For Text in Time Widget",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { md: "14px", lg: "14px", xl: "14px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   infoStatusWidget:{
//     description: "For Text in Time Widget",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { md: "14px", lg: "14px", xl: "14px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },

//   // FAB
//   floatingButtonText:{
//     description: "For Text in FAB",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { base: "14px", sm: "14px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },

//   // Story Box
//   headingTextStoryBoxEdit:{
//     description: "For Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { base: "24px", sm: "28px", md: "30px", lg: "32px", xl: "34px" },
//       lineHeight: "40px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   subHeadingTextStoryBoxEdit:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "16px", sm: "16px", md: "18px", lg: "20px", xl: "20px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   textStoryBoxEdit:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "16px", sm: "16px", md: "16px", lg: "16px", xl: "16px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   ButtonStoryBoxEdit:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { base: "14px", sm: "16px", md: "16px", lg: "16px", xl: "16px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   placeholderStoryBoxEdit:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "14px", sm: "16px", md: "16px", lg: "16px", xl: "16px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },

//   // History
//   buttonHistory:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "14px", sm: "16px", md: "16px", lg: "16px", xl: "16px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   selectHistory:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "14px", sm: "14px", md: "14px", lg: "14px", xl: "14px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   summaryHistory:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "14px", sm: "16px", md: "16px", lg: "16px", xl: "16px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   headingHistoryList:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "700",
//       fontSize: { base: "18px", sm: "18px", md: "18px", lg: "18px", xl: "18px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   contentHistoryList:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "14px", sm: "14px", md: "14px", lg: "14px", xl: "14px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   infoTodayHistoryList:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "12px", sm: "14px", md: "14px", lg: "14px", xl: "14px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
//   badgeHistoryList:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "500",
//       fontSize: { base: "12px", sm: "12px", md: "12px", lg: "12px", xl: "12px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },

//   // Footer
//   textFooter:{
//     description: "For Sub Heading Text in Story Box",
//     value: {
//       fontFamily: "var(--font-quicksand)",
//       fontWeight: "600",
//       fontSize: { base: "14px", sm: "14px", md: "14px", lg: "14px", xl: "14px" },
//       lineHeight: "20px",
//       letterSpacing: "0px",
//       textDecoration: "none",
//       textTransform: "none",
//     },
//   },
// })
import { defineTextStyles } from '@chakra-ui/react'

export const textStyles = defineTextStyles({

  // Landing Page
  headingLP:{
    description: 'For Heading in Landing Page',
    value: {
      fontFamily: 'var(--font-playfair-display)',
      fontWeight: '600',
      fontSize: { base: '50px', md: '50px' },
      lineHeight: '24px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  subHeadingLP:{
    description: 'For Sub Heading in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '400',
      fontSize: '35px',
      lineHeight: '40px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  subHeadingLP2:{
    description: 'For Sub Heading in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '35px',
      lineHeight: '40px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  textLP:{
    description: 'For Sub Heading in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: '18px',
      lineHeight: '40px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  textLP2:{
    description: 'For Sub Heading in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '18px',
      lineHeight: '40px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  headLPGrid:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '20px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'uppercase',
    },
  },
  textLPGrid:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: '16px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  headingLPLogin:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '20px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  textLPLogin:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: '14px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  googleLPLogin:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: '14px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  miniHeadingLPFeat:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '14px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'Uppercase',
    },
  },
  headingLPFeat:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '16px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  textLPFeat:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: '14px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  headingLPAbout:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '20px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'uppercase',
    },
  },
  subHeadingLPAbout:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '18px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'uppercase',
    },
  },
  textLPAbout:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: '16px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  textLPAbout2:{
    description: 'For Heading Feature Grid in Landing Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: '16px',
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'uppercase',
    },
  },

  // Navbar
  headingNav:{
    description: 'For Heading in Navbar',
    value: {
      fontFamily: 'var(--font-playfair-display)',
      fontWeight: '600',
      fontSize: { base: '22px', sm: '26px', md: '28px', lg: '30px', xl: '40px' },
      lineHeight: '24px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  subheadingNav:{
    description: 'For Sub Heading in Navbar',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { md: '12px', lg: '14px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  selectNav:{
    description: 'For Text Menu in Navbar',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '14px', sm: '14px', md: '14px', lg: '14px', xl: '14px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },

  // Time Widget
  timeWidgetNum:{
    description: 'For Number in Time Widget',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '60px', sm: '60px', md: '60px', lg: '60px', xl: '60px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  timeWidgetText:{
    description: 'For Text in Time Widget',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '24px', sm: '28px', md: '28px', lg: '28px', xl: '35px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },

  // Status Widget
  headingStatusWidget:{
    description: 'For Text in Time Widget',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: { base: '600', sm: '600', md: '700', lg: '700', xl: '700' },
      fontSize: { md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  contentStatusWidget:{
    description: 'For Text in Time Widget',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { md: '14px', lg: '14px', xl: '14px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  infoStatusWidget:{
    description: 'For Text in Time Widget',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { md: '14px', lg: '14px', xl: '14px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },

  // FAB
  floatingButtonText:{
    description: 'For Text in FAB',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '14px', sm: '14px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },

  // Story Box
  headingTextStoryBoxEdit:{
    description: 'For Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '24px', sm: '28px', md: '30px', lg: '32px', xl: '34px' },
      lineHeight: '40px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  subHeadingTextStoryBoxEdit:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '16px', sm: '16px', md: '18px', lg: '20px', xl: '20px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  textStoryBoxEdit:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '16px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  ButtonStoryBoxEdit:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '14px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  placeholderStoryBoxEdit:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '14px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },

  // History
  buttonHistory:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '14px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  selectHistory:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '14px', sm: '14px', md: '14px', lg: '14px', xl: '14px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  summaryHistory:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '14px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  headingHistoryList:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '700',
      fontSize: { base: '18px', sm: '18px', md: '18px', lg: '18px', xl: '18px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  contentHistoryList:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '14px', sm: '14px', md: '14px', lg: '14px', xl: '14px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  infoTodayHistoryList:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '12px', sm: '14px', md: '14px', lg: '14px', xl: '14px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  badgeHistoryList:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '500',
      fontSize: { base: '12px', sm: '12px', md: '12px', lg: '12px', xl: '12px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },

  // Footer
  textFooter:{
    description: 'For Sub Heading Text in Story Box',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '14px', sm: '14px', md: '14px', lg: '14px', xl: '14px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },

  //Not Authorized
  headingNotAuth:{
    description: 'For Heading Text in Not Authorized',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: { base: '22px', sm: '24px', md: '24px', lg: '24px', xl: '24px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'uppercase',
    },
  },
  subHeadingNotAuth:{
    description: 'For Sub Heading Text in Not Authorized',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '16px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  textNotAuth:{
    description: 'For Text in Not Authorized',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '16px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  countNotAuth:{
    description: 'For Countdown in Not Authorized',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '26px', sm: '26px', md: '28px', lg: '28px', xl: '28px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  buttonNotAuth:{
    description: 'For Button text in Not Authorized',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '14px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },


  // Profile Page
  headingProfile:{
    description: 'For Heading in Profile Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '800',
      fontSize: { base: '18px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  subHeadingProfile:{
    description: 'For Sub Heading in Profile Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '14px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  headingSheetsProfile:{
    description: 'For Heading Sheets in Profile Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '16px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  linkSheetsProfile:{
    description: 'For Link Text in Profile Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '16px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },
  buttonProfile:{
    description: 'For Button text in Profile Page',
    value: {
      fontFamily: 'var(--font-quicksand)',
      fontWeight: '600',
      fontSize: { base: '16px', sm: '16px', md: '16px', lg: '16px', xl: '16px' },
      lineHeight: '20px',
      letterSpacing: '0px',
      textDecoration: 'none',
      textTransform: 'none',
    },
  },

})
